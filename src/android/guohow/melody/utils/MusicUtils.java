...
public class MusicUtils {
	...
	public static String getAlbumArt(String trackId, Context context) {
		...
		final String mUriTrack = "content://media/external/audio/media/#";
		String[] projection = new String[] { "album_id" };
		final String selection = "_id = ?";
		final String[] selectionArgs = new String[] { trackId };
		...
		final String mUriAlbums = "content://media/external/audio/albums";
	...
	public static List<Mp3Bean> getMp3Infos(Cursor cursor, Context context) {
		...
		final List<Mp3Bean> mp3Infos = new ArrayList<Mp3Bean>();
		for (int i = 0; i < cursor.getCount(); i++) {
			cursor.moveToNext();
			final long id = cursor.getLong(cursor
					.getColumnIndex(BaseColumns._ID)); // 音乐id
			...
			final String artist = cursor.getString(cursor
					.getColumnIndex(AudioColumns.ARTIST));// 艺术
			...
			final int isMusic = cursor.getInt(cursor
					.getColumnIndex(AudioColumns.IS_MUSIC));//
			if (isMusic != 0) {

				final Mp3Bean mp3Info = new Mp3Bean();
	...
	public static void listUpdate(List<Mp3Bean> mp3Infos) {
		...
		MusicUtils.mp3list = new ArrayList<HashMap<String, Object>>();
		for (final Iterator iterator = mp3Infos.iterator(); iterator.hasNext();) {
			final Mp3Bean mp3Info = (Mp3Bean) iterator.next();
